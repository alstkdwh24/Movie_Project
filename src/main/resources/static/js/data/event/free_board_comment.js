document.addEventListener('DOMContentLoaded', (event) => {
    ...
    // // let usernameFree=document.getElementById("usernameFree").value;
    // // let voUsername=document.getElementById("voUsername").value;
    // // if (usernameFree === voUsername) {
    // talk_button_update.onclick = function () {
    //     document.free_detail_two.action = "/free_detail_update";
    //     document.free_detail_two.submit();
    //     document.free_detail_two.method = "get"
    // }
    // }else{
    //     location.href="/movie/community/freeboard";
    // }
    const free_number = document.getElementById("freenumber");
    const talk_button_cancel = document.getElementById("talk_button_cancel_two");
    talk_button_cancel.onclick = function () {
        $.ajax({
            type: "get",


            url: `talk_button_cancel/${free_number}`,
            success: function () {
            location.href = "movie/community/freeboard";
            },
            error: function (jqXHR, textStatus, errorThrown) {
                alert("AJAX 요청 실패: " + textStatus);
            }
        });
    }
...
function create_comment(data) {
    ...
    data.forEach(function (result) {
        movie_comment_list += '<div id="talk_writer_free" >' +
            ...
            ' </div> ' +
            '<input type="hidden" id="comment_number" value="' + result.comment_number + '">' +
...
let hi = document.getElementById("freenumber");
console.log("his", hi.value);
