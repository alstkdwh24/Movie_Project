...
button_submit.onclick=function () {
    if(photo.length > 0){
        for(let i = 0; i < photo.length; i++){
            ...
            let delicious_filename=sessionStorage.getItem("data");
            console.log("delicious_filename",delicious_filename);

            let formData=new FormData();
            let uploadPaths="";

            let delicious_name=document.getElementById("resist_title").value;
            let resist_textarea=document.getElementById("resist_textarea").value;

            formData.append("delicious_name",delicious_name);
            formData.append("resist_textarea",resist_textarea);
            formData.append("delicious_filename",delicious_filename);
            formData.append("delicious_file",photo_file);
            formData.append("uploadPaths",uploadPaths);

            $.ajax({
                type:"POST",
                url: "/movie_resist/DeliciousVO_resist",
                data: formData,
                processData:false,
                contentType: false,
                success:function () {
